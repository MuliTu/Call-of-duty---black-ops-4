...
class StatfulLogin extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            user: [],
    ...
    async componentDidMount() {
        if (localStorage.getItem('token') !== null) {
            ...
            this.setState({
                isToken: true,
                redirect: generatePath('user/:id/platform/:platform', {
                    id: this.state.query,
    ...
    saveToken = () => localStorage.setItem('token', `${this.state.query},${this.state.platform}`);

    render() {
        const {valid, query, redirect, isToken} = this.state;
        const path = `/user/${this.state.query}/platform/${this.state.platform}`;
        return (
            <div className='login'>
                {
                    isToken ?
